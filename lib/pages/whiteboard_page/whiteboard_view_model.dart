...
import 'package:amber/models/draw/point.dart';
import 'package:amber/models/draw/whiteboard_content.dart';
...
class WhiteboardViewModel extends ChangeNotifier {
  ...
  void onGestureUpdate(Point point, Point previousPoint) {
    if (_tool == Tool.pen) {
      ...
      _remove(point, point2: previousPoint);
    }
  }
  ...
  void onGestureEnd() {
    if (_tool == Tool.pen) {
      ...
      _currentLine = null;
      _writtenIds.add(id);
      _whiteboardRef.set(_content.toJson());
    }
  }
    ...
    final lines = _content.lines.where((line) {
      if (line.points.length == 1) {
        ...
        for (var i = 0; i < line.points.length - 1; i++) {
          if (_rectanglesIntersect(line.points[i], line.points[i + 1], point1, point2!)) {
            return false;
          }
        }
        return true;
  ...
  bool _rectangleContainsPoint(Point rectangle1, Point rectangle2, Point contained) =>
  ...
  void _onContentSnapshot(DocumentSnapshot snapshot) {
    ...
    final content = WhiteboardContent.fromJson(snapshot.data as Map);
